...
import ru.habrahabr.arlidor.objects.MP3Player;
...
//        }
    ...
    private void processSeek(double bytes) {
        try {
            ...
            Logger.getLogger(MP3PlayerGui.class.getName()).log(java.util.logging.Level.SEVERE, null, e);
            movingFromJump = false;
        }
    ...
    private void btnDeleteSongActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteSongActionPerformed
        int[] indexPlayList = lstPlayList.getSelectedIndices();// получаем выбранные индексы(порядковый номер) песен
        if (indexPlayList.length > 0) {// если выбрали хотя бы одну песню
            ArrayList<MP3> mp3ListForRemove = new ArrayList<>();// сначала сохраняем все mp3 для удаления в отдельную коллекцию
    ...
    public static void main(String args[]) {
        try {
            ...
            Logger.getLogger(MP3PlayerGui.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //</editor-fold>
